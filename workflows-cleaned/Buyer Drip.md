...
async function sendEmail(base44, to, subject, body) {
  ...
  try {
    ...
    const rawEmail = [`To: ${to}`, `From: ${JACOB_EMAIL}`, `Subject: ${subject}`, `MIME-Version: 1.0`, `Content-Type: text/plain; charset=utf-8`, ``, body].join("\r\n");
...
async function runBuyerDrip(base44) {
  ...
  const due = allBuyers.filter(b => b.next_followup_date <= today && !["Inactive"].includes(b.status) && b.email);

  for (const buyer of due) {

    try {

      const touchNum = (buyer.touch_count || 0) + 1;

      if (touchNum > 5) { results.skipped++; continue; }

      const typeTemplates = BUYER_TEMPLATES[buyer.buyer_type] || BUYER_TEMPLATES["default"];
  ...
  await sendEmail(base44, JACOB_EMAIL, `💰 Buyer Drip: ${results.contacted} buyers contacted today`,

    `💰 Buyer Outreach Sequence Run\n\nTotal due: ${due.length}\n✅ Contacted: ${results.contacted}\n⏭️ Skipped: ${results.skipped}\n\nView buyer list: ${SITE}/ReferralEngine`);

  return Response.json({ success: true, ...results, total_due: due.length });
