...
export default function HQLiveTicker({ leads, followUps, deals }) {
  ...
  useEffect(() => {
    ...
    hotLeads.forEach(l => events.push({ icon: '⚡', text: `HOT: ${l.name} — Score ${l.motivation_total_score || '?'}/20`, color: '#f39c12' }));

    const overdue = followUps.filter(f => f.scheduled_date < today && f.status === 'Scheduled');
    ...
    closedDeals.forEach(d => events.push({ icon: '💰', text: `CLOSED: ${d.property_address} — $${(d.assignment_fee || 0).toLocaleString()} PROFIT`, color: '#27ae60' }));

    const underContract = deals.filter(d => d.status === 'Under Contract');

    underContract.forEach(d => events.push({ icon: '📝', text: `UNDER CONTRACT: ${d.property_address}`, color: '#9b59b6' }));
