...
export default function HQPipelineFunnel({ leads }) {
  ...
  const avgScore = leads.filter(l => l.motivation_total_score > 0).length > 0

    ? Math.round(leads.filter(l => l.motivation_total_score > 0).reduce((s, l) => s + (l.motivation_total_score || 0), 0) / leads.filter(l => l.motivation_total_score > 0).length * 10) / 10
