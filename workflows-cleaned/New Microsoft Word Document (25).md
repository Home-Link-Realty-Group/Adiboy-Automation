...
function CRMDashboardComponent({ leads, deals, followUps, onCall }) {
  ...
  const highPriority = leads.filter((l) => (l.motivation_total_score || 0) >= 15 && !['Dead', 'Closed'].includes(l.status));

  const mustPursue = leads.filter((l) => (l.motivation_total_score || 0) >= 18);
            ...
            {[...overdueFU, ...todayFU].slice(0, 6).map((f) => (
              ...
              <div key={f.id} className="flex justify-between text-sm">
                ...
                <span>
                  ...
                  <span className="text-slate-500"> — Touch #{f.touch_number}</span>

                </span>

                <span className={`text-xs font-bold ${f.scheduled_date < today ? 'text-red-500' : 'text-green-600'}`}>

                  {f.scheduled_date < today ? '⚠️ OVERDUE' : 'Today'}
