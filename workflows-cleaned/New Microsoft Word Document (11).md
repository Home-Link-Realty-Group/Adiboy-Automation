...
export default function CityInsightsPanel({ cities, summary }) {
  ...
  if (summary.top_performer) {
    ...
    insights.push({
      ...
      title: `${summary.top_performer.city} leads in traffic`,

      body: `${summary.top_performer.sessions.toLocaleString()} sessions — your strongest city. Double down on this market with more content + ad spend.`,

    });

  }

  if (summary.best_converter) {

    insights.push({

      icon: "💎", color: "#7c3aed",

      title: `${summary.best_converter.city} converts at ${summary.best_converter.rate}%`,
  ...
  if (noTraffic.length > 0) {
    ...
    insights.push({
      ...
      title: `${summary.no_traffic_cities} cities have zero traffic`,

      body: `${noTraffic.map(c => c.city).join(", ")}${summary.no_traffic_cities > 5 ? "..." : ""} — pages exist but invisible. Need indexing, internal links, or backlinks.`,
