...
// Plaid Connect card — opens Plaid Link, exchanges public_token, lists connected banks.
...
export default function PlaidConnectCard({ userEmail, onChange }) {
  ...
  const [syncing, setSyncing] = useState(null); // plaid_item_id being synced
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [its, accs] = await Promise.all([
        ...
        base44.entities.PlaidItem.filter({ user_email: userEmail }),

        base44.entities.BankAccount.filter({ user_email: userEmail }),
  ...
  async function handleConnect() {
    ...
    try {
      ...
      const res = await base44.functions.invoke('plaidCreateLinkToken', { user_email: userEmail });
      ...
      const handler = window.Plaid.create({
        ...
        onSuccess: async (public_token, metadata) => {
          ...
          try {
            ...
            await base44.functions.invoke('plaidSyncTransactions', {
              ...
              plaid_item_id: eData.plaid_item_id,
        ...
        onExit: (err) => {
          ...
          if (err) setError(err.error_message || err.display_message || '');
  ...
  async function handleSync(item) {
    ...
    try {
      ...
      setLastSyncResult({ institution: item.institution_name, ...data });
  ...
  async function handleUnlink(item) {
    ...
    if (!window.confirm(`Disconnect ${item.institution_name}? This deletes synced transactions for this bank.`)) return;
  ...
  const totalBalance = accounts
    ...
    .filter(a => a.is_active && a.type === 'depository')
      ...
      {loading ? (
        ...
        <div style={{ background: '#f8fafc', border: '2px dashed #cbd5e1', borderRadius: 10, padding: 32, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 14 }}>
            ...
            Click <strong>Connect Bank</strong> to securely link your accounts. In sandbox mode, search for "First Platypus Bank" and use credentials <code>user_good</code> / <code>pass_good</code>.
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {items.map(item => {
            ...
            const itemAccounts = accounts.filter(a => a.plaid_item_id === item.plaid_item_id);

            const isSyncing = syncing === item.plaid_item_id;
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontWeight: 800, color: '#0B1F45' }}>{item.institution_name}</span>
                    ...
                    <div style={{ fontSize: 11, color: '#64748b', marginTop: 3 }}>
                      ...
                      {itemAccounts.length} account{itemAccounts.length !== 1 ? 's' : ''} · Last synced {item.last_sync_at ? new Date(item.last_sync_at).toLocaleString() : 'never'}
                    ...
                    {itemAccounts.map(a => (
                      ...
                      <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 10px', background: '#f8fafc', borderRadius: 6, fontSize: 12 }}>
                        ...
                        <span style={{ fontWeight: 800, color: a.type === 'credit' ? '#dc2626' : '#16a34a' }}>
                          ...
                          ${(a.current_balance || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
