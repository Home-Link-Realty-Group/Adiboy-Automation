...
const ACCENT_OPTIONS = ['#D4A843', '#0B1F45', '#16a34a', '#7b2d8b', '#dc2626', '#0891b2', '#d97706', '#ec4899'];
  ...
  return (
    ...
    <SectionCard icon={Eye} accent="#8b5cf6" tint="#ede9fe" title="Display & Region" description="How information looks and which formats to use">
      ...
      <SettingRow title="UI Density" description="How tightly information packs in lists and tables"
        ...
        <select value={prefs.ui_density} onChange={e => onChange({ ui_density: e.target.value })} style={inputSty}>
          ...
          {ACCENT_OPTIONS.map(c => (
            ...
            <button key={c} onClick={() => onChange({ accent_color: c })}
              ...
              style={{
                ...
                background: c, border: prefs.accent_color === c ? '3px solid #fff' : '2px solid transparent',

                outline: prefs.accent_color === c ? `2px solid ${c}` : 'none',
      ...
      <SettingRow title="Date Format"
        ...
        <select value={prefs.date_format} onChange={e => onChange({ date_format: e.target.value })} style={inputSty}>
      ...
      <SettingRow title="First Day of Week" description="Used in calendars and weekly views"
        ...
        <select value={prefs.first_day_of_week} onChange={e => onChange({ first_day_of_week: e.target.value })} style={inputSty}>
