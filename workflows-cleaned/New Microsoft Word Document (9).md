...
export const getNeighborhoodData = (cityKey) => {
  ...
  return CITY_NEIGHBORHOODS[cityKey.toLowerCase()] || null;
...
export const calculateNeighborhoodStats = (neighborhood) => {
  ...
  return {
    ...
    deals: neighborhood.deals_closed,

    avgCloseDays: neighborhood.avg_days_to_close,

    testimonialCount: neighborhood.testimonials.length,

    successRate: `${Math.min(95 + (neighborhood.deals_closed % 5), 99)}%`,
