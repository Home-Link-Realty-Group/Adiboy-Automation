...
export default function PhasePageShell({
  ...
  const idx = ALL_PHASES.findIndex(p => p.id === phaseId);

  const prev = idx > 0 ? ALL_PHASES[idx - 1] : null;

  const next = idx < ALL_PHASES.length - 1 ? ALL_PHASES[idx + 1] : null;
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          {ALL_PHASES.map(p => {
            ...
            const passed = ALL_PHASES.findIndex(x => x.id === p.id) < idx;
