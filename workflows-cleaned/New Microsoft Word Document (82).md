  ...
  if (!stripePromise) {
    ...
    stripePromise = base44.functions.invoke('getStripePublishableKey', {})
      ...
      .then(res => {
        ...
        if (!data.publishable_key) throw new Error('Stripe key not configured');
 ...
 * Resolves a subscription's pending_setup_intent on-session.

 *

 * Per Stripe docs (Set up payment methods for subscriptions with no initial payment):

 *   - status === 'requires_action' → call confirmCardSetup with existing card

 *   - status === 'requires_payment_method' → collect a NEW card, then update sub
...
export default function PaymentSetupResolver({ userEmail, onComplete }) {
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      const res = await base44.functions.invoke('getSubscriptionSetupIntent', { user_email: userEmail });
  ...
  // needs_action — render Stripe Elements provider
...
function ResolveForm({ status, userEmail, clientSecret, onSuccess }) {
  ...
  const isAuthFailure = status === 'requires_action';

  const isPMFailure = status === 'requires_payment_method';
