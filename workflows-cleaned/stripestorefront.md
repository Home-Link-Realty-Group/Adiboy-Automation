...
export default function StripeConnectStorefront() {
  ...
  async function loadStore() {
    ...
    try {
      ...
      const statusRes = await base44.functions.invoke('connectAccountStatus', { account_id: accountId });

      const sd = statusRes.data || statusRes;

      if (sd.ok && sd.account?.display_name) setMerchantName(sd.account.display_name);
  ...
  async function handleBuy(product) {
    ...
    try {
      ...
      const res = await base44.functions.invoke('connectCheckout', {
        ...
        application_fee_cents: calcAppFee(product.default_price.unit_amount),
            ...
            {products.map(p => (
              ...
              <div key={p.id} style={{ background: '#fff', borderRadius: 14, padding: 22, border: '1.5px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
                ...
                <button
                  ...
                  disabled={buying === p.id || !p.default_price}
