...
export default function ListsPanel({ onClose }) {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const [l, t] = await Promise.all([List.list("-sort_order", 100), EnterpriseTask.list()]);
  ...
  async function createList() {
    ...
    const newList = await List.create({
      ...
      color: LIST_COLORS.find(c => c.name === listColor)?.hex || "#3b82f6",
  ...
  async function addTask() {
    ...
    const task = await EnterpriseTask.create({
      ...
      subtasks: newTaskSubtasks.map(s => ({ id: `sub_${Date.now()}_${Math.random()}`, name: s, completed: false })),

      list_id: selectedList.id,

    });

    const updatedList = { ...selectedList, task_ids: [...(selectedList.task_ids || []), task.id] };

    await List.update(selectedList.id, { task_ids: updatedList.task_ids });
  ...
  async function deleteTask(taskId) {
    ...
    if (selectedList) {
      ...
      const updated = selectedList.task_ids.filter(id => id !== taskId);

      await List.update(selectedList.id, { task_ids: updated });

      setSelectedList({ ...selectedList, task_ids: updated });
  ...
  function exportList() {
    ...
    const listTasks = tasks.filter(t => selectedList.task_ids?.includes(t.id));
  ...
  const listTasks = selectedList ? tasks.filter(t => selectedList.task_ids?.includes(t.id)) : [];
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {lists.map(list => (
            ...
            <div
              ...
              <div style={{ fontSize: 11, color: "#888", marginLeft: 28 }}>{tasks.filter(t => list.task_ids?.includes(t.id)).length} tasks</div>
