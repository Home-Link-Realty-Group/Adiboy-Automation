...
export default function EditDocumentModal({ doc, onClose, onSave }) {
  ...
  // On open: if no saved content yet but a file_url exists, auto-extract text from the file
  ...
  async function extractTextFromFile() {
    ...
    try {
      ...
      if (result?.status === "success" && result?.output?.full_text) {
        ...
        setContent(result.output.full_text);

      } else if (result?.status === "success" && Array.isArray(result.output) && result.output[0]?.full_text) {

        setContent(result.output[0].full_text);
  ...
  async function saveAsCopy() {
    ...
    await base44.entities.StoredDocument.create({
      ...
      file_size: ((content.length / 1024).toFixed(2)) + " KB",

      file_type: "text/plain",

      file_name: title + " (Copy).txt"
        ...
        <div style={header}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#666" }}>{doc.file_name || doc.title}</div>
