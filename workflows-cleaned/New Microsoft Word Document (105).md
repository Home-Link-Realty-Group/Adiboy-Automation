...
import { PHONE, PHONE_RAW } from "./portalConstants";
  ...
  async function uploadFiles(files) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        done.push({ name: files[i].name, url: file_url, size: files[i].size });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 20, padding: "36px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ marginTop: 20, background: "#f0f9ff", borderRadius: 12, padding: "14px 18px", fontSize: 13, color: "#1e3a5f" }}>
          ...
          📱 Or text photos directly to <a href={`sms:${PHONE_RAW}`} style={{ color: "#2563eb", fontWeight: 700 }}>{PHONE}</a> — we'll attach them to your file automatically.
