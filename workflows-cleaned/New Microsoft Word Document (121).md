...
export default function AnalyticsPanel({ posts, campaigns }) {
  ...
  const byType = useMemo(() => {
    ...
    posts.forEach(p => { counts[p.post_type || 'Custom'] = (counts[p.post_type || 'Custom'] || 0) + 1; });
  ...
  const byPlatform = useMemo(() => {
    ...
    return Object.entries(counts).map(([name, value]) => ({ name: name.charAt(0).toUpperCase() + name.slice(1), value, fill: PLATFORM_COLORS[name] || '#666' }));
  ...
  const byStatus = useMemo(() => {
    ...
    return Object.entries(counts).map(([name, value]) => ({ name, value, fill: STATUS_COLORS[name] || '#666' }));
