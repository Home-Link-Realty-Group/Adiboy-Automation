...
const PRIORITY_ORDER = { 'CALL_NOW': 0, 'TODAY': 1, 'THIS_WEEK': 2, 'MONITOR': 3 };
    ...
    try {
      ...
      if (response.scoredLeads) {
        ...
        const sorted = response.scoredLeads.sort((a, b) => {
          ...
          const priorityDiff = (PRIORITY_ORDER[a.callPriority] || 999) - (PRIORITY_ORDER[b.callPriority] || 999);
...
function LeadCard({ lead, expanded, setExpanded }) {
  ...
  const hotness = HOTNESS_COLORS[lead.hotnessLevel] || HOTNESS_COLORS.COOL;
