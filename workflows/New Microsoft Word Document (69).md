...
  const totalRevenue = closed\.reduce\(\(s, d\) => s \+ \(d\.assignment\_fee || 0\), 0\);

  const thisMonthRev = closed

    \.filter\(d => d\.closing\_date?\.startsWith\(thisMonth\) || d\.created\_date?\.startsWith\(thisMonth\)\)

    \.reduce\(\(s, d\) => s \+ \(d\.assignment\_fee || 0\), 0\);

  const lastMonthRev = closed

    \.filter\(d => d\.closing\_date?\.startsWith\(lastMonth\) || d\.created\_date?\.startsWith\(lastMonth\)\)

    \.reduce\(\(s, d\) => s \+ \(d\.assignment\_fee || 0\), 0\);

  const pendingValue = \[\.\.\.underContract, \.\.\.buyerFound\]\.reduce\(\(s, d\) => s \+ \(d\.assignment\_fee || 0\), 0\);
...
      \.filter\(dl => dl\.closing\_date?\.startsWith\(key\) || dl\.created\_date?\.startsWith\(key\)\)

      \.reduce\(\(s, dl\) => s \+ \(dl\.assignment\_fee || 0\), 0\);
