...
  const \{ connected, label, purpose, setup\_url, critical, platform\_managed, details \} = integration;
...
    const saved = localStorage\.getItem\(\`hl\_int\_enabled\_$\{userEmail\}\`\);
...
    const key = \`hl\_int\_enabled\_$\{userEmail\}\`;
...
      \{\!connected && setup\_url && \(

        <a href=\{setup\_url\} style=\{\{
