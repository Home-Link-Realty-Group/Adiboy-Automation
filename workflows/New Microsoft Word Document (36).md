...
  \{ key: 'is\_pre\_foreclosure', label: 'Pre\-Foreclosure', icon: '🔴' \},

  \{ key: 'has\_tax\_lien', label: 'Tax Lien', icon: '🔴' \},

  \{ key: 'has\_code\_violation', label: 'Code Violation', icon: '🟡' \},

  \{ key: 'is\_vacant', label: 'Vacant', icon: '🟠' \},

  \{ key: 'is\_absentee', label: 'Absentee Owner', icon: '🟠' \},

  \{ key: 'is\_free\_and\_clear', label: 'Free & Clear', icon: '🟢' \},
...
  if \(lead\.is\_pre\_foreclosure\) count\+\+;

  if \(lead\.has\_tax\_lien\) count\+\+;

  if \(lead\.has\_code\_violation\) count\+\+;

  if \(lead\.is\_vacant\) count\+\+;

  if \(lead\.is\_absentee\) count\+\+;

  if \(lead\.distress\_flags\) count \+= lead\.distress\_flags\.split\(','\)\.filter\(Boolean\)\.length;
...
    \.filter\(\(l\) => \(l\.stacked\_flags\_count || getStackedCount\(l\)\) >= 2\)
...
                      \{DISTRESS\_FLAGS\.filter\(\(f\) => l\[f\.key\]\)\.map\(\(f\) => \(
