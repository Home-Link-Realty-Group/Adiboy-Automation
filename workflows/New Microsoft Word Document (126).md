...
const TEXT\_POSITIONS = \['Top Left', 'Top Center', 'Top Right', 'Center', 'Bottom Left', 'Bottom Center', 'Bottom Right'\];

const FONTS = \['inherit', 'Georgia', 'Arial Black', 'Trebuchet MS', 'Impact', 'Courier New', 'Palatino'\];

const BRAND\_TEMPLATES = \[
...
                  \{TEXT\_POSITIONS\.map\(p => <option key=\{p\} value=\{p\}>\{p\}</option>\)\}
