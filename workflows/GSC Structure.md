...
    const \{ page\_route, page\_url \} = await req\.json\(\);

    if \(\!page\_route || \!page\_url\) \{

      return Response\.json\(\{ error: 'page\_route and page\_url required' \}, \{ status: 400 \}\);

    \}

    const gscData = await fetchGSCData\(page\_url\);

    const lighthouseData = await runLighthouseAudit\(page\_url\);
...
    const lighthouseApiUrl = \`https://www\.googleapis\.com/pagespeedonline/v5/runPagespeed?url=$\{encodeURIComponent\(pageUrl\)\}&key=$\{Deno\.env\.get\('LIGHTHOUSE\_API\_KEY'\)\}\`;
