...
const SKIP\_URL = "https://the\-replicator\-bfa0beaa\.base44\.app/functions/apifySkipTrace";
...
  const raw\_headers = lines\[0\]\.split\(","\)\.map\(h => h\.replace\(/^"|"$/g, ""\)\.trim\(\)\.toLowerCase\(\)\);
...
  a\.download = \`skip\_trace\_$\{new Date\(\)\.toISOString\(\)\.slice\(0,10\)\}\.csv\`;
