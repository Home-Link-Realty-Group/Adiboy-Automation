...
1\. Update \`CACHE\_VERSION\` in public/sw\.js
...
navigator\.serviceWorker\.controller?\.postMessage\(\{ type: 'CLEAR\_CACHE' \}\);
...
\- Track cache hit rate via \`from\_cache\` event \(custom\)
...
   \- Update \`CACHE\_VERSION = 'v0\-disabled'\`
