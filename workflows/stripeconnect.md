...
// in Stripe Dashboard → Products\. Or use stripe\_create\_product/stripe\_create\_price tools\.

const PLATFORM\_SUB\_PRICE\_ID = 'price\_REPLACE\_WITH\_YOUR\_PLATFORM\_PRICE\_ID';
...
    const saved = localStorage\.getItem\('homelink\_user\_email'\);
...
        user\_email: userEmail\.toLowerCase\(\),
...
        await refreshStatus\(acct\.stripe\_account\_id\);

        await loadProducts\(acct\.stripe\_account\_id\);
...
      const res = await base44\.functions\.invoke\('connectAccountStatus', \{ account\_id: accountId \}\);
...
    localStorage\.setItem\('homelink\_user\_email', clean\);
...
        contact\_email: contactEmail || email,

      \}\);

      const data = res\.data || res;

      if \(data\.ok && data\.onboarding\_url\) \{

        // Redirect to Stripe\-hosted onboarding

        window\.location\.href = data\.onboarding\_url;
...
        account\_id: account\.stripe\_account\_id,
...
        price\_cents: Math\.round\(parseFloat\(newProduct\.price\) \* 100\),
...
        await loadProducts\(account\.stripe\_account\_id\);
...
        price\_id: PLATFORM\_SUB\_PRICE\_ID,
...
  const ready = status?\.ready\_to\_process\_payments;

  const onboardingComplete = status?\.onboarding\_complete;
...
                Account: <code style=\{code\}>\{account\.stripe\_account\_id\}</code> · Status:\{' '\}

                <strong>\{status?\.requirements\_status || 'loading…'\}</strong>

              </div>

            </div>

            <button onClick=\{\(\) => refreshStatus\(account\.stripe\_account\_id\)\} style=\{btnGhost\} title="Refresh from Stripe">
...
              <a href=\{\`/StripeConnect/Store/$\{account\.stripe\_account\_id\}\`\} style=\{\{ \.\.\.btnPrimary, marginTop: 14, textDecoration: 'none' \}\}>
...
            \{account\.subscription\_status === 'active' ? \(
...
                    Renews: \{account\.subscription\_current\_period\_end ? new Date\(account\.subscription\_current\_period\_end\)\.toLocaleDateString\(\) : '—'\}
