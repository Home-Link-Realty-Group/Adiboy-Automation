...
const STATUS\_COLORS = \{ Active: '\#27ae60', Paused: '\#f39c12', Completed: '\#3498db', Draft: '\#9b59b6' \};
...
  const \[form, setForm\] = useState\(\{ name: '', goal: '', platforms: 'Facebook, Instagram', start\_date: '', end\_date: '', notes: '', status: 'Draft' \}\);

  const \[saving, setSaving\] = useState\(false\);

  const \[plannerForm, setPlannerForm\] = useState\(\{ campaign\_goal: '', platforms: 'Facebook, Instagram', duration\_days: '30', post\_frequency: '5 posts per week' \}\);
...
    setForm\(\{ name: '', goal: '', platforms: 'Facebook, Instagram', start\_date: '', end\_date: '', notes: '', status: 'Draft' \}\);
...
    if \(\!plannerForm\.campaign\_goal\) return;
...
      const res = await socialMediaManager\(\{ action: 'plan\_campaign', \.\.\.plannerForm \}\);
...
  const getCampaignPosts = \(campaignId\) => posts\.filter\(p => p\.campaign\_id === campaignId\);
...
              <div key=\{c\.id\} style=\{\{ background: 'rgba\(255,255,255,0\.04\)', border: \`1px solid $\{STATUS\_COLORS\[c\.status\]\}30\`, borderTop: \`3px solid $\{STATUS\_COLORS\[c\.status\]\}\`, borderRadius: 12, padding: 18 \}\}>
...
                  <span style=\{\{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 8, background: STATUS\_COLORS\[c\.status\] \+ '25', color: STATUS\_COLORS\[c\.status\] \}\}>\{c\.status\}</span>

                </div>

                <div style=\{\{ display: 'grid', gridTemplateColumns: 'repeat\(3, 1fr\)', gap: 8, marginBottom: 12 \}\}>

                  \{\[\['Posts', cPosts\.length\], \['Reach', \(c\.total\_reach || 0\)\.toLocaleString\(\)\], \['Leads', c\.total\_leads || 0\]\]\.map\(\(\[label, val\]\) => \(
...
                \{c\.start\_date && <div style=\{\{ fontSize: 10, color: '\#555', marginBottom: 10 \}\}>📅 \{c\.start\_date\}\{c\.end\_date ? \` → $\{c\.end\_date\}\` : ''\}</div>\}
...
              \{ label: 'Start Date', field: 'start\_date', type: 'date' \},

              \{ label: 'End Date', field: 'end\_date', type: 'date' \},
...
                  \{ label: 'Campaign Goal', field: 'campaign\_goal', placeholder: 'e\.g\. Generate motivated seller leads for Dallas TX market' \},

                  \{ label: 'Target Platforms', field: 'platforms', placeholder: 'Facebook, Instagram' \},

                  \{ label: 'Duration \(days\)', field: 'duration\_days', placeholder: '30' \},

                  \{ label: 'Post Frequency', field: 'post\_frequency', placeholder: '5 posts per week' \},
...
                <button onClick=\{planCampaign\} disabled=\{planning || \!plannerForm\.campaign\_goal\} style=\{\{ width: '100%', background: planning ? '\#333' : '\#3498db', border: 'none', color: '\#fff', borderRadius: 8, padding: '13px', fontWeight: 900, fontSize: 13, cursor: planning ? 'not\-allowed' : 'pointer', marginTop: 6 \}\}>
...
                  <div style=\{\{ fontSize: 14, fontWeight: 900, color: '\#3498db', marginBottom: 6 \}\}>\{generatedPlan\.campaign\_name\}</div>
...
                  \{\(generatedPlan\.weekly\_themes || \[\]\)\.map\(\(wt, i\) => \(
...
                  <div style=\{\{ fontSize: 11, fontWeight: 700, color: '\#888', textTransform: 'uppercase', marginBottom: 8 \}\}>Post Ideas \(\{\(generatedPlan\.post\_ideas || \[\]\)\.length\}\)</div>

                  <div style=\{\{ maxHeight: 280, overflowY: 'auto' \}\}>

                    \{\(generatedPlan\.post\_ideas || \[\]\)\.map\(\(pi, i\) => \(
...
                      name: generatedPlan\.campaign\_name,

                      goal: plannerForm\.campaign\_goal,
