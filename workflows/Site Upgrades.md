...
Also changed \`send\_page\_view: true\` → \`send\_page\_view: false\` \(prevents duplicate tracking since React/hooks handle pageviews\):
...
\- \[x\] \`send\_page\_view: false\` set \(prevent double tracking\)
