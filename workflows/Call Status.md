...
        updates\.seller\_motivation\_score = Math\.min\(\(lead\.seller\_motivation\_score || 5\) \+ 1, 10\);

      \}

      if \(recordingUrl\) \{

        updates\.call\_recording\_url = recordingUrl;
...
              Authorization: \`Basic $\{btoa\(\`$\{TWILIO\_SID\}:$\{TWILIO\_AUTH\}\`\)\}\`,
