...
    CITY\_PAGES\.foreclosure\.pages\.forEach\(\(page\) => \{

      if \(contentLower\.includes\(page\.city\.toLowerCase\(\)\)\) \{

        suggestions\.push\(\{

          anchor\_text: page\.name,

          target\_url: page\.path,
...
    CITY\_PAGES\.inherited\.pages\.forEach\(\(page\) => \{
...
  CITY\_PAGES\.general\.pages\.forEach\(\(page\) => \{
...
    \.map\(\(s\) => \`\- \[$\{s\.anchor\_text\}\]\($\{s\.target\_url\}\)\`\)
...
    \["click here", "read more", "link"\]\.includes\(l\.anchor\_text\.toLowerCase\(\)\)
...
    internal\_links: internalLinks\.length,

    external\_links: externalLinks\.length,
