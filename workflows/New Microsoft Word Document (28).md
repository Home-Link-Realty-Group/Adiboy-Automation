...
        <div className="text\-sm font\-semibold">\{f\.lead\_name\}</div>

        <div className="text\-xs text\-slate\-500">Touch \#\{f\.touch\_number\} • \{f\.method\}</div>

      </div>

      <span className=\{\`text\-xs font\-bold $\{overdue ? 'text\-red\-500' : 'text\-green\-600'\}\`\}>

        \{overdue ? \`⚠️ $\{f\.scheduled\_date\}\` : 'Today'\}
