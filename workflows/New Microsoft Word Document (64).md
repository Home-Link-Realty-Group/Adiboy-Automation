...
    \.filter\(f => f\.scheduled\_date < today && f\.status === 'Scheduled'\)
...
              const daysOverdue = Math\.floor\(\(new Date\(\) \- new Date\(f\.scheduled\_date\)\) / \(1000 \* 60 \* 60 \* 24\)\);
...
                      <div style=\{\{ fontSize: 11, fontWeight: 700, color: '\#fff' \}\}>\{f\.lead\_name || 'Unknown'\}</div>

                      <div style=\{\{ fontSize: 9, color: '\#888' \}\}>Touch \#\{f\.touch\_number\} — \{f\.method\}</div>
...
                      <div style=\{\{ fontSize: 8, color: '\#666' \}\}>\{f\.scheduled\_date\}</div>
