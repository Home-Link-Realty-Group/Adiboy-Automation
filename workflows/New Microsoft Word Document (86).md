...
    try \{ return localStorage\.getItem\('saas\_sidebar\_collapsed'\) === '1'; \} catch \{ return false; \}
...
    try \{ localStorage\.setItem\('saas\_sidebar\_collapsed', collapsed ? '1' : '0'\); \} catch \{\}
...
    ? NAV\_GROUPS\.map\(g => \(\{ \.\.\.g, items: g\.items\.filter\(i => i\.label\.toLowerCase\(\)\.includes\(q\)\) \}\)\)\.filter\(g => g\.items\.length\)
