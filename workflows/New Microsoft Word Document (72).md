...
  \{ name: 'Twilio Voice', key: 'twilio\_voice', icon: '📞', desc: 'Outbound calling bridge', configured: true \},

  \{ name: 'Twilio SMS', key: 'twilio\_sms', icon: '💬', desc: 'SMS outreach & alerts', configured: true \},
...
  // Secrets confirmed: TWILIO\_ACCOUNT\_SID, TWILIO\_AUTH\_TOKEN, RESEND\_API\_KEY, APIFY\_API\_TOKEN
...
    const d = new Date\(l\.created\_date || 0\);

    return \(Date\.now\(\) \- d\.getTime\(\)\) < 30 \* 24 \* 60 \* 60 \* 1000; // last 30 days

  \}\);

  const hasAnyTouches = leads\.some\(l => \(l\.touch\_count || 0\) > 0\);
...
    twilio\_voice: hasAnyTouches ? 'active' : 'configured',

    twilio\_sms:   hasAnyTouches ? 'active' : 'configured',
