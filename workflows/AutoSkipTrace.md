...
const APIFY\_API\_KEY = Deno\.env\.get\('APIFY\_API\_TOKEN'\);

const APIFY\_ACTOR\_ID = 'apify/skip\-tracer\-api';
...
        const apifyRes = await fetch\(\`https://api\.apify\.com/v2/acts/$\{APIFY\_ACTOR\_ID\}/run\-sync?token=$\{APIFY\_API\_KEY\}\`, \{
