...
          description="Once logged in, click your avatar \(top right\) → 'Settings' → 'Integrations'\. You'll see a section called 'Personal API tokens'\. Click 'Show' on the default token \(or create a new one named 'ProFlow CRM'\)\. Copy the entire token — it's a long string starting with 'apify\_api\_\.\.\.'\. Treat it like a password\."
...
              placeholder="apify\_api\_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
